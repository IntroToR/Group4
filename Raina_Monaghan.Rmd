...
ggplot(all_cheese_long) + geom_col(aes(x = lbs_per_person, y = cheese_type, fill = cheese_type)) + labs(x = "Year", y = "Dairy Type") + facet_wrap(~Year) + labs ( main = "Cheese Consumption Trends - Time Series") +
...
ggplot(clean_cheese,
       aes(x = clean_cheese$Year,
           ...
           color = "Total Processed Cheese")) + geom_point() +
  geom_smooth(method = lm,
              se = TRUE,
              fullrange = TRUE) + 
  labs(title = "Total Processed Cheese Consumption", x = "Year", y = "Consumption (lbs)") + theme_bw()
